...
export function registerIpc(controller: AppController, getWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle(IPC.ScriptsSetPinned, (_e, pid: string, sid: string, pinned: boolean) =>
    controller.setScriptPinned(pid, sid, pinned)
  )
  ipcMain.handle(IPC.ScriptsRunInTerminal, (_e, pid: string, sid: string, appId: string) =>
    controller.runScriptInTerminal(pid, sid, appId)
  )
